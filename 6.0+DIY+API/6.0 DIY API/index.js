...
app.get("/filter", (req, res) => {
  ...
  const jokeTypeFound = jokes.filter(
    (filteredJoke) => filteredJoke.jokeType === typeOfJoke
  );
  console.log(jokeTypeFound);
  res.json(jokeTypeFound);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  const foundJokeId = jokes.findIndex((joke) => joke.id === jokeId);
  const newJokeObj = { id: jokeId, jokeText: newJoke, jokeType: typeOfJoke };
  console.log(req.params);
  res.json((jokes[foundJokeId] = newJokeObj));
});

app.patch("/jokes/:id", (req, res) => {
  // step 1 get new Joke data
  const jokeId = parseInt(req.params.id);
  const updatedJokeText = req.body.text;
  const updatedJokeType = req.body.type;
  console.log(req.body)
  // step 2 find the joke to be updated

  const jokeToUpdate = jokes.find(joke => joke.id === jokeId);
  const newJoke = {
    id:jokeId,
    jokeText:updatedJokeText ?updatedJokeText :jokeToUpdate.jokeText,
    jokeType: updatedJokeType? updatedJokeType: jokeToUpdate.jokeType

  };
  // step 3 find the joke index
...
app.delete("/jokes/:id", (req, res) => {
  const jokeId = parseInt(req.params.id);
  const searchIndex = jokes.findIndex(joke=> joke.id === jokeId)
  const updatedArray = jokes.splice(searchIndex, 1);
  console.log('deleted Joke', jokeId)
